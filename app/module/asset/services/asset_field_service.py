...
from app.module.asset.constant import DEFAULT_ASSET_FIELD
...
class AssetFieldService:
    async def get_asset_field(self, session: AsyncSession, user_id: int) -> list[str]:
        ...
        if asset_field is None:
            await AssetFieldRepository.save(session, AssetField(user_id=user_id, field_preference=DEFAULT_ASSET_FIELD))
            return DEFAULT_ASSET_FIELD
        else:
