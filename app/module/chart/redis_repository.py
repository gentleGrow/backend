...
class RedisRichPickRepository:
    @staticmethod
    async def get(redis_client: Redis, key: str) -> dict[str, str] | list[str] | None:
        rich_people = await redis_client.get(key)
        return json.loads(rich_people) if rich_people is not None else None
