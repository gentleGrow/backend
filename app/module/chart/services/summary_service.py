...
from app.common.util.time import get_date_past_day, check_date_weekend
...
class SummaryService:
    ...
    def get_today_review_rate(
        ...
        past_assets = [
            asset
            for asset in assets
            if asset.asset_stock.trade_date <= get_date_past_day(PAST_MONTH_DAY)
        ]
        ...
        past_total_amount = self.asset_service.get_total_asset_amount(
            past_assets, past_stock_map, exchange_rate_map
        )
    ...
    async def get_past_stock_map(self, session:AsyncSession, assets:list[Asset], lastest_stock_daily_map:dict[str, StockDaily]) -> dict[str, float]:
        past_date = self._get_weekday_date(PAST_MONTH_DAY)
        past_stock_daily_map:dict[tuple[str, date], StockDaily] =  await self.stock_daily_service.get_date_map(session, assets, past_date)
        ...
        for stock_code in stock_codes:
            ...
            past_price = stock_daily.adj_close_price
